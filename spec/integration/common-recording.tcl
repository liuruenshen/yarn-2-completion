...
set printable_string_pat [string cat {[} "\x20-\x7e" {]} "*"]

proc simulate_user_type {command} {
  global printable_string_pat
  set chars [split $command ""]
  foreach {char} $chars {
    send -- "$char"
    if {[string match $printable_string_pat $char]} {
      expect -exact "$char"
      after [expr {int(50 + rand() * 100)}]
    } else {
      after [expr {int(50 + rand() * 100)}]
    }
  }
...
proc send_and_expect [list \
  ...
  #send -- $send_command
  simulate_user_type $send_command
  expect {
    -- $send_command {
